...
def main():
    ...
    while True:
        try:
            ...
            # 自动检查Token使用情况
            token_count = manager.gpt_client.count_messages_tokens()
            max_tokens = manager.gpt_client.max_tokens
            usage_percent = (token_count / max_tokens) * 100
            
            # 如果Token使用率超过80%，自动提醒用户
            if usage_percent > 80:
                console.print(f"[red]⚠️  当前Token使用率: {usage_percent:.1f}% ({token_count}/{max_tokens})[/red]")
                console.print("[red]建议使用 /reset 重置对话历史以避免API错误[/red]")
                if usage_percent > 95:
                    if Confirm.ask("Token使用率过高，是否自动重置对话历史？"):
                        manager.reset_conversation()
                        console.print("[green]对话历史已自动重置[/green]")
            elif usage_percent > 60:
                console.print(f"[yellow]Token使用率: {usage_percent:.1f}%[/yellow]")
            ...
            if user_input.lower() == '/quit':
                ...
                console.print(f"[blue]Token使用情况：[/blue]")
                console.print(f"当前使用: {token_count} tokens")
                console.print(f"最大限制: {max_tokens} tokens")
                console.print(f"使用率: {usage_percent:.1f}%")
                
                if usage_percent > 80:
                    console.print("[red]⚠️  Token使用率过高，建议使用/reset重置对话历史[/red]")
                elif usage_percent > 60:
                    console.print("[yellow]⚠️  Token使用率较高，注意监控[/yellow]")
                else:
                    console.print("[green]✓ Token使用率正常[/green]")
                continue
